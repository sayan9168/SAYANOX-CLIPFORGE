...
type RenderClip = {
  ...
  score: number | null; vertical: boolean; captions: boolean; bytes: number; download: string; thumbnail?: string;
};
type JobStatus = {
...
export default function Home() {
  ...
  const [trims, setTrims] = useState<{ start: number; end: number }[]>([]);
    ...
    pollTimer.current = setInterval(async () => {
      try {
        ...
        if (j.status === "completed") {
          ...
          setHighlights(clips);
          setSelected(clips.map(() => true));
          setTrims(clips.map((c) => ({ start: c.start, end: c.end })));
          setRendered([]);
  ...
  async function analyze() {
    ...
    try {
      ...
      if (d.mode === "demo") {
        const demo = d.clips.map((c: DemoClip) => ({
        ...
        setTrims(demo.map((c: Highlight) => ({ start: c.start, end: c.end })));
  ...
  async function render() {
    ...
    try {
      const picks = highlights
        .map((h, i) => ({
          ...h,
          start: trims[i]?.start ?? h.start,
          end: trims[i]?.end ?? h.end,
          locked: true,
          selected: selected[i] !== false,
        }))
        .filter((h) => h.selected);
      if (!picks.length) { setError("Select at least one clip."); setPhase("ready"); return; }
      const r = await fetch("/api/jobs/render", {
        ...
        body: JSON.stringify({
          ...
          highlights: picks,
          durations: durations.length ? durations : picks.map((h) => Math.max(5, Math.round(h.end - h.start))),
  ...
  const thumbSrc = (c: RenderClip) => c.thumbnail ? `/api/jobs/files?job=${renderJob}&path=${encodeURIComponent("clips/" + c.thumbnail)}` : "";
    ...
    <main>
      <nav>
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
          ...
          <div className="status"><i /> v0.9 • picker + hook</div>
        </div>
      </nav>
      ...
      <section className="hero">
        ...
        <p className="sub">Pick clips, trim them, copy YT/IG/TikTok captions in EN/BN/HI, then render with a 2s hook title.</p>
        ...
        <div className="features">
          <div><Zap size={18} /><b>Pick + trim</b><small>Select clips and lock exact in/out times</small></div>
          <div><Clock3 size={18} /><b>YT / IG / TikTok</b><small>EN · বাংলা · हिंदी captions + hashtags</small></div>
          <div><Film size={18} /><b>Hook + thumb</b><small>First 2s title burn and JPEG cover</small></div>
        </div>
        ...
        {(phase === "ready" || phase === "rendering" || phase === "done") && highlights.length > 0 && (
          <div className="results">
            <div className="resultHead">
              ...
              <span>{selected.filter(Boolean).length}/{highlights.length} selected</span>
            </div>
            ...
            <Phase2Toolbar
              ...
              onRender={render}
              highlightCount={Math.max(1, selected.filter(Boolean).length || highlights.length)}
            />
            <ClipPicker
              clips={highlights}
              selected={selected.length === highlights.length ? selected : highlights.map(() => true)}
              setSelected={setSelected}
              trims={trims.length === highlights.length ? trims : highlights.map((c) => ({ start: c.start, end: c.end }))}
              setTrims={setTrims}
              platform={platform}
              busy={busy}
            />
            ...
            <div className="grid">
              {rendered.map((c) => (
                <article className="genClip" key={c.file}>
                  <video controls preload="metadata" playsInline poster={thumbSrc(c) || undefined} src={clipSrc(c)} />
                  <div className="genMeta">
                    ...
                    <div className="genActions">
                      ...
                      {c.thumbnail && <a href={thumbSrc(c)} download={c.thumbnail} className="dl ghost">Thumb</a>}
                    </div>
                  </div>
                </article>
              ))}
      ...
      <footer>SAYANOX CLIPFORGE · v0.9 picker + hook</footer>
    </main>
  );
}
