...
def handle_render(job: dict, work: Path, progress) -> dict:
    ...
    for idx, hl in enumerate(items):
        ...
        extra_vf: list[str] = []
        if params.get("grade"):
            extra_vf.append("eq=contrast=1.08:saturation=1.15:brightness=0.02")
        if params.get("hook_zoom"):
            extra_vf.append("scale=iw*1.1:ih*1.1,crop=iw/1.1:ih/1.1")
        _render_final(
            raw_out,
            staged if (bgm_path and use_bgm) else final,
            info, center_x, render_vf, aspect, work, extra_vf,
        )
...
def _render_final(raw: Path, dst: Path, info: dict, center_x, vf_captions: list[str], aspect: str, work: Path, extra_vf: list[str] | None = None) -> None:
    ...
    chain = ",".join(vf + (extra_vf or []) + vf_captions + post)
    args = ["-i", str(raw)]
    if chain:
